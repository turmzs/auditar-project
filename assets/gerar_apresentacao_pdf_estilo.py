...
def atualizar_cores(cores_personalizadas):
    ...
    if cores_personalizadas:
        ...
        CORES["COR_PRETO"] = cores_personalizadas.get(
            "texto", RGBColor(0x2D, 0x2D, 0x2D)
        )
        CORES["COR_DOURADO"] = cores_personalizadas.get(
            "accent", RGBColor(0xB8, 0x8F, 0x00)
        )
        CORES["COR_BRANCO"] = cores_personalizadas.get(
            "fundo", RGBColor(0xFF, 0xFF, 0xFF)
        )
        CORES["COR_CINZA_ESC"] = cores_personalizadas.get(
            "primaria", RGBColor(0x4A, 0x4A, 0x4A)
        )
        CORES["COR_CINZA_MED"] = cores_personalizadas.get(
            "texto_secundario", RGBColor(0x7A, 0x7A, 0x7A)
        )
        CORES["COR_TEXTO_MUTED"] = cores_personalizadas.get(
            "texto_secundario", RGBColor(0xA0, 0xA0, 0xA0)
        )
        ...
        print(
            f"🎨 Cores atualizadas: Fundo={CORES['COR_BRANCO']}, Destaque={CORES['COR_DOURADO']}"
        )
...
def adicionar_logo_auditar(slide, x=0.3, y=4.6, largura=0.8):
    ...
    try:
        logo_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "logo_auditar.png"
        )
...
def slide_capa_pdf(prs, nome_empresa, responsavel, data_emissao):
    ...
    adicionar_texto(
        slide,
        "ANÁLISE ESTRATÉGICA E PROJEÇÕES",
        0.3,
        1.3,
        9.4,
        0.5,
...
def slide_dre_pdf(prs, dados_mensais, cenarios):
    ...
    adicionar_texto(
        slide,
        "📊 DRE COMPARATIVO ENTRE CENÁRIOS",
        1.3,
        0.02,
        8,
        0.6,
    ...
    dados_cenarios = [
        ...
        (
            cenarios[0]["receita_proj"],
            cenarios[0]["custo_proj"],
            cenarios[0]["lucro_proj"],
            cenarios[0]["imposto_proj"],
            cenarios[0]["lucro_liq_proj"],
        ),
        (
            cenarios[1]["receita_proj"],
            cenarios[1]["custo_proj"],
            cenarios[1]["lucro_proj"],
            cenarios[1]["imposto_proj"],
            cenarios[1]["lucro_liq_proj"],
        ),
        (
            cenarios[2]["receita_proj"],
            cenarios[2]["custo_proj"],
            cenarios[2]["lucro_proj"],
            cenarios[2]["imposto_proj"],
            cenarios[2]["lucro_liq_proj"],
        ),
    ]
...
def slide_plano_acao_pdf(prs, nome_empresa, cenarios):
    ...
    adicionar_texto(
        slide,
        "KPIs DE SUCESSO: Redução 15% custos | Aumento 25% eficiência | Economia R$ 1.5M tributos",
        0.5,
        4.45,
        8.4,
        0.3,
    ...
    cenarios = [
        {
            ...
            "lucro_liq_proj": ((receita_atual * 1.3) - (custos_atual * 1.1))
            - ((receita_atual * 1.3) * 0.06),
        },
        {
            "nome": "Realista",
            ...
            "lucro_liq_proj": ((receita_atual * 1.1) - (custos_atual * 1.05))
            - ((receita_atual * 1.1) * 0.06),
        },
        {
            "nome": "Pessimista",
            ...
            "lucro_liq_proj": ((receita_atual * 0.8) - (custos_atual * 0.95))
            - ((receita_atual * 0.8) * 0.06),
        },
    ]
...
def calcular_comparacao_periodos(dados_mensais):
    ...
    for i, d in enumerate(dados_mensais):
        ...
        if i > 0 and lucro_anterior:
            var_lucro = (
                ((lucro_atual - lucro_anterior) / lucro_anterior * 100)
                if lucro_anterior > 0
                else 0
            )
        else:
    ...
    var_receita_total = (
        (
            (ultimo["receita_bruta"] - primeiro["receita_bruta"])
            / primeiro["receita_bruta"]
            * 100
        )
        if primeiro["receita_bruta"] > 0
        else 0
    )
    var_lucro_total = (
        (
            (ultimo["lucro_operacional"] - primeiro["lucro_operacional"])
            / primeiro["lucro_operacional"]
            * 100
        )
        if primeiro["lucro_operacional"] > 0
        else 0
    )
...
Responda de forma profissional e concisa:"""
    ...
    try:
        ...
        api_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "..", "data", "api_config.json"
        )
...
def gerar_apresentacao_pptx_pdf(
    ...
    logo_existe = os.path.exists(
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "logo_auditar.png")
    )
