...
class GeradorIAInteligente:
    ...
    async def gerar_apresentacao_inteligente(
        ...
        print(
            f"Design escolhido: {requisitos_design.get('nome_estilo', 'Personalizado')}"
        )

        # 2. Gerar código Python dinâmico para os slides
        codigo_gerado = await self._gerar_codigo_slides(
            dados_mensais, nome_empresa, responsavel, requisitos_design, num_slides
        )

        # 3. Executar código gerado
        print("Construindo apresentacao...")
        filepath = await self._executar_codigo_gerado(
            codigo_gerado, dados_mensais, nome_empresa, responsavel, requisitos_design
        )
    ...
    async def _call_ollama_chat(self, messages: List[Dict[str, str]]) -> str:
        ...
        try:
            ...
            response = await self.client.post(
                f"{OLLAMA_HOST}/api/chat", json=payload, timeout=300.0
            )
    ...
    async def analisar_financas(
        ...
        for d in dados_mensais[-6:]:
            resumo.append(
                f"Mês {d['mes']}/{d['ano']}: Receita R$ {d['receita_bruta']:,.2f}, Lucro R$ {d['lucro_operacional']:,.2f}"
            )

        dados_texto = "\n".join(resumo)

        instrucao = f"Você é um consultor financeiro brasileiro. Analise estes dados da empresa {nome_empresa} e forneça um diagnóstico estratégico curto (máximo 2 parágrafos) com 1 tendência e 2 sugestões estratégicas. Responda em Português, ignore Inglês e Espanhol. Não repetir  tópicos."
        instrucao += "\nNÃO explique o significado dos termos (ex: não explique o que é redução de custos), apenas dê as sugestões práticas."
        instrucao += "\nCriar exemplos dentro das sugestões para deixar mais claro. Não mostrar as instruções nos diagnósticos. Seja apenas direto."
    ...
    async def _analisar_requisitos(self, comando: str) -> Dict[str, Any]:
        ...
        try:
            ...
            messages = [
                {
                    "role": "system",
                    "content": "Voce e um designer. Responda APENAS o JSON com cores RGB baseadas no comando.",
                },
                {
                    "role": "user",
                    "content": f'Command: "{comando}". Fill this JSON with colors: {{"nome_estilo":"","cores":{{"fundo":[255,255,255],"titulo":[0,0,0],"texto":[0,0,0],"acento":[0,0,0],"destaque":[0,0,0]}},"fontes":{{"titulo":"Calibri","corpo":"Calibri"}},"estilo_visual":"","elementos_especiais":[],"layout":"","descricao":""}}',
                },
            ]
            ...
            if "vermelho" in cmd_l or "red" in cmd_l:
                response = response.replace("[0,0,0]", "[220,53,69]").replace(
                    "[0, 0, 0]", "[220,53,69]"
                )
            elif "azul" in cmd_l or "blue" in cmd_l:
                response = response.replace("[0,0,0]", "[0,112,192]").replace(
                    "[0, 0, 0]", "[0,112,192]"
                )
            elif "verde" in cmd_l or "green" in cmd_l:
                response = response.replace("[0,0,0]", "[40,167,69]").replace(
                    "[0, 0, 0]", "[40,167,69]"
                )
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        cores_encontradas = re.findall(
                            r"\[(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\]", json_str
                        )
            ...
            if requisitos is None:
                print(
                    "[AVISO] Nao foi possivel extrair JSON, usando fallback inteligente"
                )
                return self._design_fallback(comando)
    ...
    async def _gerar_codigo_slides(
        ...
        resumo_dados = {
            ...
            "margem": f"{margem:.1f}%",
            "primeiro_mes": (
                fmt_mes_ano(dados_mensais[0].get("mes"), dados_mensais[0].get("ano"))
                if dados_mensais
                else ""
            ),
            "ultimo_mes": (
                fmt_mes_ano(dados_mensais[-1].get("mes"), dados_mensais[-1].get("ano"))
                if dados_mensais
                else ""
            ),
        }
...
CÓDIGO PYTHON:"""
        ...
        if not self.ollama_available:
            return self._codigo_fallback(
                design, nome_empresa, responsavel, resumo_dados, dados_mensais
            )
        ...
        try:
            messages = [
                {
                    "role": "system",
                    "content": "Voce e um programador Python expert em python-pptx. Gere APENAS codigo Python executavel, sem explicacoes.",
                },
    ...
    async def _executar_codigo_gerado(
        ...
        try:
            ...
            arquivo_recente = self._encontrar_arquivo_recente(
                nome_empresa, tempo_limite_segundos=10
            )
            if arquivo_recente:
    ...
    def _extrair_json(self, texto: str) -> str:
        ...
        json_str = re.sub(
            r'"fundo":\s*(\d+),\s*(\d+),\s*(\d+)', r'"fundo": [\1, \2, \3]', json_str
        )
        json_str = re.sub(
            r'"titulo":\s*(\d+),\s*(\d+),\s*(\d+)', r'"titulo": [\1, \2, \3]', json_str
        )
        json_str = re.sub(
            r'"texto":\s*(\d+),\s*(\d+),\s*(\d+)', r'"texto": [\1, \2, \3]', json_str
        )
        json_str = re.sub(
            r'"acento":\s*(\d+),\s*(\d+),\s*(\d+)', r'"acento": [\1, \2, \3]', json_str
        )
        json_str = re.sub(
            r'"destaque":\s*(\d+),\s*(\d+),\s*(\d+)',
            r'"destaque": [\1, \2, \3]',
            json_str,
        )
    ...
    def _validar_requisitos(self, req: Dict, comando: str = "") -> Dict:
        ...
        usar_preto = (
            "preto" in comando_lower
            or "black" in comando_lower
            or "texto preto" in comando_lower
        )
        usar_branco = (
            "branco" in comando_lower
            or "white" in comando_lower
            or "fundo branco" in comando_lower
        )
    ...
    def _design_fallback(self, comando: str) -> Dict:
        ...
        if usar_preto:
            ...
            "cinza" in comando_lower
            or "gray" in comando_lower
            or "grey" in comando_lower
        ):
    ...
    def _codigo_fallback(
        self, design, nome_empresa, responsavel, resumo, dados_mensais=None
    ) -> str:
        ...
        if dados_mensais:
            for d in dados_mensais[:10]:
                ...
                custo_total = (
                    d.get("custos", 0) + d.get("despesas", 0) + d.get("impostos", 0)
                )
                custo = f"R$ {custo_total:,.0f}"
                lucro = f"R$ {d.get('lucro_operacional', 0):,.0f}"
                linhas_tabela.append(
                    f'{{"mes": "{mes}", "receita": "{rec}", "custo": "{custo}", "lucro": "{lucro}"}}'
                )
        dados_tabela = (
            ", ".join(linhas_tabela)
            if linhas_tabela
            else '{"mes": "N/A", "receita": "R$ 0", "custo": "R$ 0", "lucro": "R$ 0"}'
        )
...
'''
    ...
    def _encontrar_arquivo_gerado(self, nome_empresa: str) -> str:
        ...
        return self._encontrar_arquivo_recente(
            nome_empresa, tempo_limite_segundos=60
        ) or os.path.join(
            os.path.expanduser("~"),
            "Desktop",
    ...
    def _encontrar_arquivo_recente(
        self, nome_empresa: str, tempo_limite_segundos: int = 60
    ) -> str:
        ...
        try:
            ...
            pptx_files = [f for f in os.listdir(desktop) if f.endswith(".pptx")]
    ...
    def _gerar_fallback_local(
            ...
            try:
                ...
                if os.path.exists(logo_path):
                    slide.shapes.add_picture(
                        logo_path, Inches(x), Inches(y), width=Inches(w)
                    )
                else:
                    tx = slide.shapes.add_textbox(
                        Inches(x), Inches(y), Inches(w), Inches(0.3)
                    )
                    tf = tx.text_frame
                    p = tf.paragraphs[0]
                    p.text = "AUDITAR"
        ...
        add_retangulo(slide, 0, 0, 10, 0.15, COR_ACENTO)
        add_texto(
            slide, "DADOS MENSAIS DETALHADOS", 0.3, 0.3, 9.4, 0.5, 22, COR_TITULO, True
        )

        # Cabeçalho da tabela
        add_retangulo(slide, 0.3, 0.9, 9.4, 0.4, COR_ACENTO)
        add_texto(
            slide, "Mês/Ano", 0.4, 0.95, 2, 0.3, 12, COR_FUNDO, True, PP_ALIGN.CENTER
        )
        add_texto(
            slide, "Receita", 2.6, 0.95, 2.2, 0.3, 12, COR_FUNDO, True, PP_ALIGN.CENTER
        )
        add_texto(
            slide, "Custos", 5.0, 0.95, 2, 0.3, 12, COR_FUNDO, True, PP_ALIGN.CENTER
        )
        add_texto(
            slide, "Lucro", 7.2, 0.95, 2.2, 0.3, 12, COR_FUNDO, True, PP_ALIGN.CENTER
        )
        ...
        add_retangulo(slide, 0, 0, 10, 0.15, COR_ACENTO)
        add_texto(
            slide, "ANÁLISE DE CENÁRIOS", 0.3, 0.3, 9.4, 0.5, 24, COR_TITULO, True
        )
        ...
        for i, (nome, var, rec, luc) in enumerate(cenarios):
            ...
            add_texto(
                slide, f"R$ {rec:,.0f}", x + 0.1, 2.1, 2.8, 0.3, 14, COR_TITULO, True
            )
        ...
        add_texto(
            slide,
            "AUDITAR - CONTABILIDADE CONSULTIVA",
            0.3,
            4.8,
            9.4,
            0.3,
...
async def gerar_apresentacao_ia(
    ...
    is_claro = (
        design["cores"]["fundo"] == [255, 255, 255]
        or "branco" in cmd_l
        or "claro" in cmd_l
    )
    ...
    if not cores_personalizadas:
        ...
        cores_personalizadas = {
            ...
            "texto_secundario": (
                RGBColor(200, 200, 200) if is_escuro else RGBColor(108, 117, 125)
            ),
        ...
        print(
            f"[DESIGN] Cores aplicadas: acento RGB({', '.join(map(str, cor_destaque))})"
        )
    else:
    ...
    if template_nome == "corporativo_escuro" and cores_escolhidas:
        ...
        if "preto" in primaria or "escuro" in primaria or "cinza" in primaria:
            cores_personalizadas = {
                ...
                "secundaria": (
                    RGBColor(220, 53, 69)
                    if "vermelho" in str(cores_escolhidas)
                    else RGBColor(40, 167, 69)
                ),
