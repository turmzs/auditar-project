...
def gerar_grafico_slide(dados_mensais, cores=None):
    ...
    ax1.plot(
        ...
        marker="o",
        label="Receita",
        color=cor_receita if isinstance(cor_receita, str) else "green",
        linewidth=2,
    )
    ax1.plot(
        meses,
        ...
        marker="s",
        label="Custos",
        color=cor_custos if isinstance(cor_custos, str) else "red",
        linewidth=2,
    )
    ax1.plot(
        meses,
        ...
        marker="^",
        label="Despesas",
        color=cor_despesas if isinstance(cor_despesas, str) else "orange",
        linewidth=2,
    )
    ...
    ax2.bar(
        meses,
        lucro,
        color=cor_lucro if isinstance(cor_lucro, str) else "blue",
        alpha=0.7,
    )
...
def adicionar_grafico_ao_slide(slide, img_buffer, x=0.5, y=1.5, width=9, height=4):
    ...
    try:
        ...
        slide.shapes.add_picture(
            tmp_path, Inches(x), Inches(y), Inches(width), Inches(height)
        )
    finally:
...
def analisar_cenario_financeiro(dados_mensais):
    ...
    if len(dados_mensais) >= 6:
        ...
        if receita_anterior > 0:
            crescimento = (
                (receita_recente - receita_anterior) / receita_anterior
            ) * 100
        else:
            crescimento = 0
    else:
    ...
    return (
        cenario,
        recomendacoes_por_cenario.get(cenario, recomendacoes_por_cenario["NEUTRO"]),
        conclusoes_por_cenario.get(cenario, conclusoes_por_cenario["NEUTRO"]),
    )
...
def adicionar_logo(slide, x=None, y=0.2, largura=0.8):
    ...
    try:
        ...
        possiveis_paths = [
            ...
            os.path.join(
                os.path.dirname(os.path.dirname(__file__)), "logo_auditar.png"
            ),
        ]
        ...
        if logo_path:
            slide.shapes.add_picture(
                logo_path, Inches(x), Inches(y), width=Inches(largura)
            )
        else:
            # Fallback texto
            txBox = slide.shapes.add_textbox(
                Inches(x), Inches(y), Inches(largura), Inches(0.4)
            )
            tf = txBox.text_frame
            p = tf.paragraphs[0]
            p.text = "AUDITAR"
...
def adicionar_header_footer(slide, cores, slide_num=None, total_slides=None):
    ...
    cor_header = cores.get(
        "header", cores.get("primaria", cores.get("accent", RGBColor(212, 175, 55)))
    )
    ...
    cor_footer = cores.get(
        "footer", cores.get("secundaria", cores.get("accent", RGBColor(212, 175, 55)))
    )
    footer = slide.shapes.add_shape(
        MSO_SHAPE.RECTANGLE, 0, Inches(5.47), Inches(10), Inches(0.15)
    )
    footer.fill.solid()
    ...
    if slide_num and total_slides:
        num_box = slide.shapes.add_textbox(
            Inches(9), Inches(5.15), Inches(0.8), Inches(0.3)
        )
...
def template_corporativo_escuro(
    prs, dados_mensais, nome_empresa, responsavel, cores=None
):
    ...
    auditar_top = slide.shapes.add_textbox(
        Inches(0.3), Inches(0.2), Inches(6), Inches(0.6)
    )
    ...
    if dados_mensais:
        ...
        for idx, dado in enumerate(dados_mensais[:6]):  # Max 6 meses no slide 2
            ...
            card = slide.shapes.add_shape(
                MSO_SHAPE.ROUNDED_RECTANGLE, x, y, Inches(4.5), Inches(1.0)
            )
            card.fill.solid()
            ...
            mes_box = slide.shapes.add_textbox(
                x + Inches(0.2), y + Inches(0.2), Inches(4.1), Inches(0.35)
            )
            ...
            rec_box = slide.shapes.add_textbox(
                x + Inches(0.2), y + Inches(0.55), Inches(2), Inches(0.3)
            )
            ...
            # Lucro
            luc_box = slide.shapes.add_textbox(
                x + Inches(2.2), y + Inches(0.55), Inches(2), Inches(0.3)
            )
    ...
    if len(dados_mensais) > 6:
        ...
        for idx, dado in enumerate(dados_mensais[6:12]):  # Meses 7-12
            ...
            # Lucro
            luc_box = slide.shapes.add_textbox(
                x + Inches(2.3), y + Inches(0.55), Inches(2), Inches(0.3)
            )
            tf = luc_box.text_frame
            p = tf.paragraphs[0]
            p.text = f"Luc: R$ {dado.get('lucro_operacional', 0):,.0f}"
    ...
    # Calcular totais
    total_receita = (
        sum(d.get("receita_bruta", 0) for d in dados_mensais) if dados_mensais else 0
    )
    total_lucro = (
        sum(d.get("lucro_operacional", 0) for d in dados_mensais)
        if dados_mensais
        else 0
    )
    ...
    for idx, (label, value, color) in enumerate(metrics):
        ...
        val_box = slide.shapes.add_textbox(
            Inches(0.5), y + Inches(0.4), Inches(4), Inches(0.6)
        )
    ...
    for idx, (nome, rec, luc, cor) in enumerate(cenarios):
        ...
        # Card do cenário
        card = slide.shapes.add_shape(
            MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.5), y, Inches(9), Inches(1.1)
        )
        card.fill.solid()
        ...
        nome_box = slide.shapes.add_textbox(
            Inches(0.7), y + Inches(0.1), Inches(8.6), Inches(0.4)
        )
        ...
        # Valores
        val_box = slide.shapes.add_textbox(
            Inches(0.7), y + Inches(0.5), Inches(8.6), Inches(0.5)
        )
        tf = val_box.text_frame
        p = tf.paragraphs[0]
    ...
    if grafico_img:
        adicionar_grafico_ao_slide(
            slide, grafico_img, x=0.5, y=1.2, width=8, height=3.5
        )
    else:
    ...
    concl_box = slide.shapes.add_textbox(
    ...
    bar2 = slide.shapes.add_shape(
        MSO_SHAPE.RECTANGLE, 0, Inches(5.47), prs.slide_width, Inches(0.15)
    )
    bar2.fill.solid()
...
def template_minimalista_branco(
    prs, dados_mensais, nome_empresa, responsavel, cores=None
):
    ...
    line2 = slide.shapes.add_shape(
        MSO_SHAPE.RECTANGLE, Inches(2), Inches(4.2), Inches(6), Inches(0.05)
    )
    line2.fill.solid()
    ...
    if dados_mensais:
        ...
        for idx, dado in enumerate(dados_mensais[:4]):
            ...
            # Card
            card = slide.shapes.add_shape(
                MSO_SHAPE.ROUNDED_RECTANGLE, x, y, Inches(4.5), Inches(2.2)
            )
            card.fill.solid()
            card.fill.fore_color.rgb = RGBColor(248, 249, 250)
            ...
            # Mês/Ano
            mes_box = slide.shapes.add_textbox(
                x + Inches(0.2), y + Inches(0.2), Inches(4.1), Inches(0.5)
            )
            tf = mes_box.text_frame
            p = tf.paragraphs[0]
            ...
            # Receita
            rec_box = slide.shapes.add_textbox(
                x + Inches(0.2), y + Inches(0.8), Inches(4.1), Inches(0.4)
            )
            tf = rec_box.text_frame
            p = tf.paragraphs[0]
            ...
            # Lucro
            luc_box = slide.shapes.add_textbox(
                x + Inches(0.2), y + Inches(1.3), Inches(4.1), Inches(0.4)
            )
            tf = luc_box.text_frame
            p = tf.paragraphs[0]
    ...
    cenario, recs_dinamicas, conclusao_dinamica = analisar_cenario_financeiro(
        dados_mensais
    )

    for slide_num, titulo in enumerate(
        ["RESUMO", "ANÁLISE", "GRÁFICO", "RECOMENDAÇÕES", "CONCLUSÃO"], 3
    ):
        slide = prs.slides.add_slide(slide_layout)
        ...
        header.fill.fore_color.rgb = (
            cores["secundaria"] if slide_num % 2 == 0 else cores["destaque"]
        )
        header.line.fill.background()

        # Título
        title_box = slide.shapes.add_textbox(
            Inches(0.5), Inches(0.25), Inches(9), Inches(0.8)
        )
        tf = title_box.text_frame
        p = tf.paragraphs[0]
        p.text = titulo
        ...
        if slide_num == 3:  # Resumo
            ...
            for i, (label, value) in enumerate(metrics):
                ...
                lbl_box = slide.shapes.add_textbox(
                    Inches(0.5), y, Inches(4), Inches(0.5)
                )
                tf = lbl_box.text_frame
                p = tf.paragraphs[0]
                p.text = label
                ...
                p.font.color.rgb = (
                    cores["secundaria"]
                    if i == 0
                    else cores["destaque"] if i == 1 else cores["texto"]
                )
            ...
            sub_box = slide.shapes.add_textbox(
                Inches(0.5), Inches(4.4), Inches(9), Inches(0.6)
            )
            tf = sub_box.text_frame
            p = tf.paragraphs[0]
            p.text = f"Responsável: {responsavel}"
...
def template_moderno_gradiente(
    prs, dados_mensais, nome_empresa, responsavel, cores=None
):
    ...
    circle = slide.shapes.add_shape(
        MSO_SHAPE.OVAL, Inches(7), Inches(-2), Inches(4), Inches(4)
    )
    circle.fill.solid()
    ...
    # Linha decorativa
    line = slide.shapes.add_shape(
        MSO_SHAPE.RECTANGLE, Inches(0.8), Inches(4), Inches(2), Inches(0.08)
    )
    line.fill.solid()
    ...
    if dados_mensais:
        ...
        for idx, (label, value, color) in enumerate(metrics):
            ...
            # Card
            card = slide.shapes.add_shape(
                MSO_SHAPE.ROUNDED_RECTANGLE, x, Inches(2), Inches(3), Inches(2)
            )
            card.fill.solid()
            ...
            bar = slide.shapes.add_shape(
                MSO_SHAPE.RECTANGLE, x, Inches(2), Inches(3), Inches(0.15)
            )
            bar.fill.solid()
            ...
            label_box = slide.shapes.add_textbox(
                x + Inches(0.2), Inches(2.4), Inches(2.6), Inches(0.5)
            )
            ...
            # Valor
            val_box = slide.shapes.add_textbox(
                x + Inches(0.2), Inches(2.9), Inches(2.6), Inches(0.8)
            )
            tf = val_box.text_frame
            p = tf.paragraphs[0]
            p.text = value
    ...
    for slide_num, titulo in enumerate(
        ["ANÁLISE", "DADOS MENSAIS", "GRÁFICO", "RECOMENDAÇÕES", "CONCLUSÃO"], 3
    ):
        slide = prs.slides.add_slide(slide_layout)
        ...
        header.fill.solid()
        header.fill.fore_color.rgb = (
            cores["primaria"] if slide_num % 2 else cores["secundaria"]
        )
        header.line.fill.background()
...
def aplicar_template(
    ...
    return template_func(
        prs, dados_mensais, nome_empresa, responsavel, cores_personalizadas
    )
