...
AUDITAR CONTABILIDADE - VersÃ£o PadrÃ£o
...
# FUNÃÃO PARA CARREGAR RECURSOS (imagens, arquivos, etc)

# Funciona tanto em desenvolvimento quanto no executÃ¡vel PyInstaller
...
def get_resource_path(relative_path):
    ...
    if hasattr(sys, "_MEIPASS"):
        ...
        # EstÃ¡ rodando do executÃ¡vel PyInstaller

        base_path = sys._MEIPASS

    else:

        # EstÃ¡ rodando em desenvolvimento
...
# Caminho da logo (jÃ¡ incluso automaticamente)
...
try:
    ...
    from assets.gerador_ia_inteligente import (
        GeradorIAInteligente,
        gerar_apresentacao_ia,
    )
...
class DatabaseManager:
    ...
    def init_database(self):
        ...
        # Verificar e adicionar colunas se nÃ£o existirem (migraÃ§Ã£o)
        ...
        if "regime_tributario" not in colunas_emp:
            ...
            cursor.execute(
                "ALTER TABLE empresas ADD COLUMN regime_tributario TEXT DEFAULT 'simples'"
            )
        ...
        # Tabela de lanÃ§amentos fiscais (entradas/saÃ­das)
        ...
        cursor.execute("""
            ...
            CREATE TABLE IF NOT EXISTS lancamentos_fiscais (
                ...
                is_credito INTEGER DEFAULT 0,  -- 1 = gera crÃ©dito, 0 = nÃ£o
        ...
        # Tabela de crÃ©ditos de impostos acumulados
        ...
        # Tabela de prejuÃ­zos fiscais para compensaÃ§Ã£o
        ...
        # Tabela de memÃ³ria de cÃ¡lculo (histÃ³rico detalhado)
        ...
        try:
            ...
            if self.use_ia and IA_INTELIGENTE_DISPONIVEL:
                ...
                if self.tema_cores == "personalizado":
                    ...
                    print("ð¨ Usando cores personalizadas com IA")
                ...
                # Usar tema de cores prÃ©-definido ou personalizadas
...
class AnalisadorIAFinanceiraThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            diagnostico = loop.run_until_complete(
                ia.analisar_financas(
                    self.dados_mensais, self.nome_empresa, self.comando_personalizado
                )
            )
            ...
            self.finished.emit(f"Erro na anÃ¡lise: {str(e)}")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("Auditar Planejamento TributÃ¡rio - v6.79.0")

        self.setGeometry(100, 100, 900, 700)

        self.db = DatabaseManager()

        # Inicializar cores padrÃ£o (para evitar erro ao gerar relatÃ³rio sem visitar aba de relatÃ³rios)

        from pptx.dml.color import RGBColor

        self.cor_fundo = RGBColor(255, 255, 255)  # Branco padrÃ£o

        self.cor_header = RGBColor(0, 0, 128)  # Azul escuro padrÃ£o

        self.cor_footer = RGBColor(0, 0, 128)  # Azul escuro padrÃ£o

        self.cor_texto = RGBColor(0, 0, 0)  # Preto padrÃ£o

        self.cor_destaque = RGBColor(212, 175, 55)  # Dourado padrÃ£o
        ...
        titulo = QLabel("Auditar Planejamento TributÃ¡rio")
        ...
        # Aplicar estilo global Cinza ClÃ¡ssico / Limpo
    ...
    def criar_aba_empresas(self):
        ...
        form.addRow("Regime TributÃ¡rio:", self.combo_regime)
    ...
    def criar_aba_dados(self):
        ...
        # --- PAINEL SUPERIOR: SeleÃ§Ã£o de empresa e entrada de dados ---

        painel_entrada = QWidget()

        painel_entrada_layout = QVBoxLayout(painel_entrada)

        painel_entrada_layout.setSpacing(8)

        painel_entrada_layout.setContentsMargins(5, 5, 5, 5)
        ...
        self.combo_empresa_dados.setMinimumWidth(250)

        selecao_layout.addWidget(self.combo_empresa_dados)

        selecao_layout.addStretch()

        painel_entrada_layout.addLayout(selecao_layout)

        # Grupo: Novo Mes (COMPACTO)

        grupo_novo = QGroupBox("Adicionar Mes")

        painel_entrada_layout.addWidget(grupo_novo)

        grid_entrada = QGridLayout(grupo_novo)

        grid_entrada.setSpacing(8)

        grid_entrada.setContentsMargins(8, 8, 8, 8)
        ...
        grid_entrada.addWidget(QLabel("Mes:"), 0, 0)

        grid_entrada.addWidget(self.spin_mes, 0, 1)

        grid_entrada.addWidget(QLabel("Ano:"), 0, 2)

        grid_entrada.addWidget(self.spin_ano, 0, 3)

        grid_entrada.setColumnStretch(4, 1)

        # Linha 2: Receita

        self.txt_receita = QLineEdit("0")

        grid_entrada.addWidget(QLabel("Receita Bruta:"), 1, 0)

        grid_entrada.addWidget(self.txt_receita, 1, 1)
        ...
        grid_entrada.addWidget(QLabel("Custos:"), 2, 0)

        grid_entrada.addWidget(QLabel("Salarios:"), 2, 1)

        grid_entrada.addWidget(self.txt_custo_salarios, 2, 2)

        grid_entrada.addWidget(QLabel("Aluguel:"), 2, 3)

        grid_entrada.addWidget(self.txt_custo_aluguel, 2, 4)

        grid_entrada.addWidget(QLabel("Outros:"), 3, 1)

        grid_entrada.addWidget(self.txt_custo_outros, 3, 2)

        grid_entrada.addWidget(QLabel("Total:"), 3, 3)

        grid_entrada.addWidget(self.txt_custos_total, 3, 4)
        ...
        grid_entrada.addWidget(QLabel("Despesas:"), 4, 0)

        grid_entrada.addWidget(QLabel("Agua/Luz:"), 4, 1)

        grid_entrada.addWidget(self.txt_despesa_agua_luz_tel, 4, 2)

        grid_entrada.addWidget(QLabel("Material:"), 4, 3)

        grid_entrada.addWidget(self.txt_despesa_material, 4, 4)

        grid_entrada.addWidget(QLabel("Outros:"), 5, 1)

        grid_entrada.addWidget(self.txt_despesa_outros, 5, 2)

        grid_entrada.addWidget(QLabel("Total:"), 5, 3)

        grid_entrada.addWidget(self.txt_despesas_total, 5, 4)

        # Linha 5: Impostos e Lucro
        ...
        grid_entrada.addWidget(QLabel("Creditos:"), 6, 0)

        grid_entrada.addWidget(self.txt_creditos, 6, 1)

        grid_entrada.addWidget(QLabel("Impostos:"), 6, 3)

        grid_entrada.addWidget(self.txt_impostos, 6, 4)

        grid_entrada.addWidget(QLabel("Lucro:"), 7, 0)

        grid_entrada.addWidget(self.txt_lucro, 7, 1)

        # BotÃµes na Linha 8

        self.btn_calcular_impostos = QPushButton("Calcular")

        self.btn_calcular_impostos.clicked.connect(self.calcular_impostos_automatico)

        self.btn_calcular_impostos.setMaximumWidth(100)
        ...
        botoes_layout.addWidget(self.btn_calcular_impostos)
        ...
        campos_calculo = [
            self.txt_receita,
            self.txt_custo_salarios,
            self.txt_custo_aluguel,
            self.txt_custo_outros,
            self.txt_despesa_agua_luz_tel,
            self.txt_despesa_material,
            self.txt_despesa_outros,
            self.txt_creditos,
        ]
        ...
        layout.addWidget(painel_entrada, 0)

        # --- PAINEL INFERIOR: Tabela de dados cadastrados (MAIOR) ---

        grupo_dados = QGroupBox("Dados Mensais Cadastrados")

        layout.addWidget(grupo_dados, 1)

        dados_layout = QVBoxLayout(grupo_dados)

        dados_layout.setContentsMargins(5, 5, 5, 5)
        ...
        self.tabela_dados.setHorizontalHeaderLabels(
            ["Mes/Ano", "Receita", "Custos", "Despesas", "Impostos", "Lucro", "Acao"]
        )

        self.tabela_dados.setMinimumHeight(200)

        dados_layout.addWidget(self.tabela_dados)

        # Criar scroll para o conteudo expandÃ­vel (Trimestrais)
        ...
        # Grupo: CÃ¡lculo Trimestral (apenas para Lucro Presumido)

        self.grupo_trimestral = QGroupBox("Calculo Trimestral - Lucro Presumido")
        ...
        self.combo_trimestre.addItem("1o Trimestre (Jan/Fev/Mar)", 1)

        self.combo_trimestre.addItem("2o Trimestre (Abr/Mai/Jun)", 2)

        self.combo_trimestre.addItem("3o Trimestre (Jul/Ago/Set)", 3)

        self.combo_trimestre.addItem("4o Trimestre (Out/Nov/Dez)", 4)
        ...
        self.combo_atividade_trimestral.addItem("Servicos (32% presuncao)", "servicos")

        self.combo_atividade_trimestral.addItem(
            "Comercio (8% IRPJ, 12% CSLL)", "comercio"
        )

        self.combo_atividade_trimestral.addItem(
            "Industria (8% IRPJ, 12% CSLL)", "industria"
        )

        self.combo_atividade_trimestral.addItem(
            "Transporte (16% presuncao)", "transporte"
        )
        ...
        receitas_trimestral_layout.addWidget(QLabel("Mes 1:"), 0, 0)
        ...
        receitas_trimestral_layout.addWidget(QLabel("Mes 2:"), 1, 0)
        ...
        receitas_trimestral_layout.addWidget(QLabel("Mes 3:"), 2, 0)
        ...
        self.lbl_resultados_trimestral.setStyleSheet(
        ...
        self.grupo_trimestral_real = QGroupBox("Calculo Trimestral - Lucro Real")
        ...
        grid_real.addWidget(QLabel("Mes"), 0, 0)
        ...
        grid_real.addWidget(QLabel("Creditos* (R$)"), 0, 4)

        self.real_inputs = []

        for i in range(1, 4):

            grid_real.addWidget(QLabel(f"Mes {i}:"), i, 0)

            rec = QLineEdit("0")
            grid_real.addWidget(rec, i, 1)

            cus = QLineEdit("0")
            grid_real.addWidget(cus, i, 2)

            des = QLineEdit("0")
            grid_real.addWidget(des, i, 3)

            cre = QLineEdit("0")
            grid_real.addWidget(cre, i, 4)
        ...
        self.lbl_resultados_real.setStyleSheet(
            "font-family: Consolas, monospace; font-size: 11px;"
        )

        layout_real.addWidget(self.lbl_resultados_real)

        layout_real.addWidget(
            QLabel("* Creditos de PIS/COFINS (compras, energia, aluguel PJ, etc.)")
        )

        layout.addWidget(scroll, 0)

        # Conectar mudanÃ§a de empresa para verificar regime

        self.combo_empresa_dados.currentIndexChanged.connect(
    ...
    def criar_aba_dashboard(self):
        ...
        selecao_layout.addWidget(QLabel("  PerÃ­odo:"))
        ...
        self.combo_periodo_dash.addItem("Ãltimo trimestre", "trimestral")

        self.combo_periodo_dash.addItem("Ãltimo ano", "anual")
        ...
        # Filtro de ano (para perÃ­odo personalizado)
        ...
        # BotÃµes de AÃ§Ã£o (Excel e IA)

        self.btn_exportar_excel = QPushButton("Exportar Excel")

        self.btn_exportar_excel.setStyleSheet(
            "background-color: #ffffff; color: black; font-weight: bold; padding: 5px;"
        )
        ...
        self.btn_analise_ia.setStyleSheet(
            "background-color: #ffffff; color: black; font-weight: bold; padding: 5px;"
        )
        ...
        try:
            ...
            # Matplotlib nÃ£o estÃ¡ instalado

            self.lbl_sem_matplotlib = QLabel(
            ...
            self.lbl_sem_matplotlib.setStyleSheet(
                "font-size: 14px; color: #dc3545; padding: 20px;"
            )
        ...
        # Label de mensagem quando nÃ£o hÃ¡ dados
    ...
    def atualizar_dashboard(self):
        ...
        # Habilitar/desabilitar filtro de ano baseado no perÃ­odo selecionado
        ...
        if periodo == "personalizado":
            ...
            # Preencher anos disponÃ­veis

            anos = self.db.fetchall(
                "SELECT DISTINCT ano FROM dados_mensais WHERE empresa_id = ? ORDER BY ano DESC",
                (empresa_id,),
            )
        ...
        if not dados:
            ...
            self.lbl_sem_dados.setText(
                "Nenhum dado mensal cadastrado para esta empresa.\nAdicione dados na aba 'Dados Mensais' primeiro."
            )
        ...
        # Inverter dados se necessÃ¡rio (para ordenaÃ§Ã£o correta no grÃ¡fico)
        ...
        # GrÃ¡fico 1: Receita vs Custos vs Despesas (interativo)
        ...
        try:
            ...
            pass  # mplcursors nÃ£o estÃ¡ instalado, grÃ¡fico sem tooltips

        ax1.set_title(
            "Receita, Custos e Despesas por MÃªs", fontsize=12, fontweight="bold"
        )
        ...
        # GrÃ¡fico 2: Lucro Operacional (interativo)

        bars = ax2.bar(
            meses, lucro, color=["green" if l >= 0 else "red" for l in lucro], alpha=0.7
        )
        ...
        ax2.set_title("Lucro Operacional por MÃªs", fontsize=12, fontweight="bold")
    ...
    def verificar_regime_para_trimestral(self):
        ...
        empresa = self.db.fetchone(
            "SELECT regime_tributario FROM empresas WHERE id = ?", (empresa_id,)
        )
    ...
    def calcular_impostos_trimestral(self):
        ...
        try:
            ...
            if receita_trimestral == 0:
                ...
                QMessageBox.warning(
                    self, "Aviso", "Digite pelo menos uma receita mensal."
                )
            ...
            resultado = calc.calcular_lucro_presumido_trimestral(
                receita_trimestral, tipo_atividade
            )
...
â Total Trimestral:              R$ {resultado['total_impostos_trimestral']:,.2f}

ð¡ MÃ©dia Mensal (provisionamento): R$ {resultado['media_mensal_impostos']:,.2f}



âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ

INFORMATIVO (NÃ£o entra no total federal)
...
âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
            ...
            QMessageBox.warning(
                self, "Erro", "Digite valores numÃ©ricos vÃ¡lidos para as receitas."
            )

    def calcular_lucro_real_trimestral_action(self):
        """AÃ§Ã£o para calcular o lucro real trimestral a partir dos inputs da interface"""
...
â TOTAL TRIMESTRAL:            R$ {res['total_impostos_trimestral']:,.2f}

ð¡ MÃ©dia Mensal:                R$ {res['media_mensal_impostos']:,.2f}
...
âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
            ...
            QMessageBox.warning(self, "Erro", "Digite valores numÃ©ricos vÃ¡lidos.")
    ...
    def criar_aba_relatorios(self):
        ...
        # Adicionar opÃ§Ãµes de cores prÃ©-definidas

        for key, (nome, _) in OPCOES_CORES.items():

            self.combo_tema.addItem(nome, key)

        # Adicionar opÃ§Ã£o de cores personalizadas

        self.combo_tema.addItem("ð¨ Cores Personalizadas", "personalizado")
        ...
        self.btn_cor_header.setStyleSheet(
            "background-color: #000080; color: white; min-width: 100px;"
        )
        ...
        self.btn_cor_footer.setStyleSheet(
            "background-color: #000080; color: white; min-width: 100px;"
        )
        ...
        self.btn_cor_texto.setStyleSheet(
            "background-color: #000000; color: white; min-width: 100px;"
        )
        ...
        self.btn_cor_destaque.setStyleSheet(
            "background-color: #D4AF37; color: white; min-width: 100px;"
        )
        ...
        # Conectar mudanÃ§a no combo para mostrar/ocultar cores personalizadas
        ...
        self.txt_comando_ia.setPlaceholderText(
            "Ex: Elegante com fundo branco, titulos em dourado..."
        )
    ...
    def atualizar_lista_empresas(self):
        ...
        empresas = self.db.fetchall(
            "SELECT id, nome, cnpj, responsavel FROM empresas ORDER BY nome"
        )
        ...
        for i, emp in enumerate(empresas):
            ...
            btn_excluir.clicked.connect(
                lambda checked, id=emp[0]: self.excluir_empresa(id)
            )
        ...
        # Verificar regime para mostrar/ocultar cÃ¡lculo trimestral
    ...
    def excluir_empresa(self, empresa_id):
        ...
        resposta = QMessageBox.question(
            self, "Confirmar", "Deseja excluir esta empresa?"
        )

        if resposta == QMessageBox.StandardButton.Yes:

            self.db.execute(
                "DELETE FROM dados_mensais WHERE empresa_id = ?", (empresa_id,)
            )
    ...
    def calcular_impostos_automatico(self):
        """Calcula impostos automaticamente baseado no regime tributÃ¡rio da empresa"""
        ...
        # Buscar regime tributÃ¡rio da empresa
        ...
        if not empresa:
            ...
            QMessageBox.warning(self, "Erro", "Empresa nÃ£o encontrada")
        ...
        try:
            ...
            QMessageBox.warning(
                self, "Erro", "Digite um valor vÃ¡lido para receita bruta"
            )
        ...
        try:
            ...
            # Para Lucro Presumido, usar 'servicos' como padrÃ£o (pode ser ajustado no futuro)
...
"""
            ...
            if regime == "simples":
                ...
                detalhes += (
                    f"Anexo: {resultado['anexo']} | Faixa: {resultado['faixa']}\n"
                )

                detalhes += f"RBT12 (Faturamento 12m):   R$ {resultado['faturamento_anual']:,.2f}\n"

                detalhes += f"AlÃ­quota Nominal:           {resultado['aliquota_nominal']:.2f}%\n"

                detalhes += f"Parcela a Deduzir (anual):  R$ {resultado['parcela_deduzir_anual']:,.2f}\n"

                detalhes += "âââââââââââââââââââââââââââââââââââââ\n"

                detalhes += f"AlÃ­quota Efetiva:           {resultado['aliquota_efetiva']:.2f}%\n"
                ...
                if resultado["iss"] > 0:
                    ...
                    detalhes += (
                        f"  ISS (incluso no DAS):    R$ {resultado['iss']:,.2f}\n"
                    )

                if resultado["icms"] > 0:

                    detalhes += (
                        f"  ICMS (incluso no DAS):   R$ {resultado['icms']:,.2f}\n"
                    )
                ...
                # Usar cÃ¡lculo profissional para Lucro Real
                ...
                detalhes += "â IMPOSTOS SOBRE FATURAMENTO â\n"

                detalhes += f"  PIS (1,65%):    DÃ©b. R$ {resultado_prof.pis_debito:,.2f} - CrÃ©d. R$ {resultado_prof.pis_credito:,.2f} = R$ {resultado_prof.pis_total:,.2f}\n"

                detalhes += f"  COFINS (7,6%):  DÃ©b. R$ {resultado_prof.cofins_debito:,.2f} - CrÃ©d. R$ {resultado_prof.cofins_credito:,.2f} = R$ {resultado_prof.cofins_total:,.2f}\n"

                detalhes += f"  ICMS:           DÃ©b. R$ {resultado_prof.icms_saida:,.2f} - CrÃ©d. R$ {resultado_prof.icms_entrada:,.2f} = R$ {resultado_prof.icms_total:,.2f}\n"

                detalhes += f"  ISS (3%):       R$ {resultado_prof.iss:,.2f}\n\n"

                # Salvar resultado para possÃ­vel exportaÃ§Ã£o
            ...
            if regime == "real":
                ...
                detalhes += (
                    f"TOTAL DE IMPOSTOS: R$ {resultado_prof.total_impostos:,.2f}\n"
                )

            else:

                detalhes += (
                    f"TOTAL DE IMPOSTOS: R$ {resultado['total_impostos']:,.2f}\n"
                )

            detalhes += "âââââââââââââââââââââââââââââââââââ"

            QMessageBox.information(self, "CÃ¡lculo de Impostos", detalhes)
    ...
    def limpar_campos_dados(self):
        """Limpa os campos de entrada apÃ³s adicionar dados"""
    ...
    def gerar_relatorio(self):
        ...
        empresa = self.db.fetchone(
            "SELECT nome, responsavel FROM empresas WHERE id = ?", (empresa_id,)
        )
        ...
        dados_formatados = [
            {
                ...
                "custo_outros": d[5],
                # Despesas detalhadas
                "despesa_agua_luz_tel": d[6],
                "despesa_material": d[7],
                "despesa_outros": d[8],
                # Totais
                "custos": d[9],
                "despesas": d[10],
                "impostos": d[11],
                "lucro_operacional": d[12],
            }
            for d in dados
        ]
        ...
        # Se tema personalizado, criar dicionÃ¡rio de cores personalizadas
    ...
    def gerar_demonstrativo_pdf_action(self):
        """AÃ§Ã£o ao clicar no botÃ£o de gerar PDF AnalÃ­tico"""
        ...
        try:
            ...
            path = gerar_relatorio_pdf(
                dados_formatados, empresa[0], empresa[1] or "NÃ£o informado"
            )
            ...
            if path:
                ...
                QMessageBox.information(
                    self,
                    "Sucesso",
                    f"Demonstrativo PDF gerado com sucesso na Ãrea de Trabalho:\n{path}",
                )
    ...
    def selecionar_cor_fundo(self):
        ...
        if cor.isValid():
            ...
            self.btn_cor_fundo.setStyleSheet(
                f"background-color: {cor.name()}; min-width: 100px;"
            )
    ...
    def selecionar_cor_header(self):
        ...
        if cor.isValid():
            ...
            self.btn_cor_header.setStyleSheet(
                f"background-color: {cor.name()}; color: white; min-width: 100px;"
            )
    ...
    def exportar_excel(self):
        ...
        # Buscar dados (mesma lÃ³gica do dashboard)
        ...
        try:
            ...
            if filename:
                ...
                QMessageBox.information(
                    self,
                    "Sucesso",
                    f"RelatÃ³rio exportado com sucesso para:\n{filename}",
                )
    ...
    def gerar_analise_ia_financeira(self):
        """Gera um diagnÃ³stico financeiro usando IA baseada nos dados do dashboard"""
        ...
        if not IA_INTELIGENTE_DISPONIVEL:
            ...
            QMessageBox.warning(
                self, "Aviso", "Funcionalidades de IA nÃ£o estÃ£o disponÃ­veis."
            )
        ...
        if not dados_raw:
            ...
            QMessageBox.warning(
                self, "Aviso", "Dados insuficientes para anÃ¡lise de IA."
            )
        ...
        default_prompt = "forneÃ§a um diagnÃ³stico estratÃ©gico curto, identifique 1 tendÃªncia clara e dÃª 2 sugestÃµes estratÃ©gicas."

        comando, ok = QInputDialog.getMultiLineText(
            self,
            "Consultoria IA",
            "O que a IA deve fazer? (Deixe como estÃ¡ para o padrÃ£o)",
            default_prompt,
        )
        ...
        self.dlg_progress_ia.setWindowTitle("AnÃ¡lise Inteligente")

        self.dlg_progress_ia.setFixedSize(300, 100)

        prog_layout = QVBoxLayout(self.dlg_progress_ia)

        prog_layout.addWidget(QLabel("O consultor IA estÃ¡ analisando seus dados..."))
        ...
        self.thread_ia = AnalisadorIAFinanceiraThread(
            dados_mensais, self.combo_empresa_dash.currentText(), comando_final
        )
    ...
    def analise_ia_concluida(self, diagnostico):
        ...
        if not diagnostico or diagnostico.strip() == "":
            ...
            diagnostico = "A IA nÃ£o conseguiu gerar uma resposta. Verifique se o Ollama estÃ¡ funcionando corretamente."

        self.exibir_analise_ia(diagnostico)

    def exibir_analise_ia(self, texto):
        """Abre um diÃ¡logo para exibir o diagnÃ³stico da IA no estilo clÃ¡ssico"""

        dlg = QDialog(self)

        dlg.setWindowTitle("RelatÃ³rio de Consultoria IA")

        dlg.setMinimumSize(600, 500)

        layout = QVBoxLayout(dlg)

        # Grupo de Resultado (Estilo ClÃ¡ssico)

        group = QGroupBox("DiagnÃ³stico EstratÃ©gico")
