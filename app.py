...
class MainWindow(QMainWindow):
    ...
    def criar_aba_dados(self):
        ...
        layout.setSpacing(10)
        scroll_layout.addWidget(grupo_dados, 2)
        ...
        self.tabela_dados.setMinimumHeight(250)
        self.tabela_dados.setToolTip(
            "* Custos e Despesas com detalhamento completo (salários, aluguel, água/luz, etc.)"
        )
        ...
        self.combo_atividade_trimestral.addItem(
            "Comércio (8% IRPJ, 12% CSLL)", "comercio"
        )
        self.combo_atividade_trimestral.addItem(
            "Indústria (8% IRPJ, 12% CSLL)", "industria"
        )
        self.combo_atividade_trimestral.addItem(
            "Transporte (16% presunção)", "transporte"
        )
        ...
        layout_real.addWidget(
            QLabel("* Créditos de PIS/COFINS (compras, energia, aluguel PJ, etc.)")
        )
    ...
    def atualizar_dashboard(self):
        ...
        ax1.set_title(
            "Receita, Custos e Despesas por Mês", fontsize=12, fontweight="bold"
        )
...
═══════════════════════════════════════════════════════════
            ...
            QMessageBox.warning(
                self, "Erro", "Digite valores numéricos válidos para as receitas."
            )
...
═══════════════════════════════════════════════════════════
    ...
    def calcular_impostos_automatico(self):
        ...
        try:
            ...
            QMessageBox.warning(
                self, "Erro", "Digite um valor válido para receita bruta"
            )
            return
...
"""
    ...
    def gerar_demonstrativo_pdf_action(self):
        ...
        try:
            path = gerar_relatorio_pdf(
                dados_formatados, empresa[0], empresa[1] or "Não informado"
            )
            if path:
                QMessageBox.information(
                    self,
                    "Sucesso",
                    f"Demonstrativo PDF gerado com sucesso na Área de Trabalho:\n{path}",
                )
        except Exception as e:
    ...
    def gerar_analise_ia_financeira(self):
        ...
        if not IA_INTELIGENTE_DISPONIVEL:
            QMessageBox.warning(
                self, "Aviso", "Funcionalidades de IA não estão disponíveis."
            )
            return
        ...
        if not dados_raw:
            QMessageBox.warning(
                self, "Aviso", "Dados insuficientes para análise de IA."
            )
            return
